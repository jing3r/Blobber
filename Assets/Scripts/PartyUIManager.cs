...
public class PartyUIManager : MonoBehaviour
{
    ...
    public Transform partyMemberInfoContainer; // Ранее назывался slotContainer
    ...
    private void CreatePartyInfoSlotsUI()
    {
        ...
        {
            if (memberStats == null) continue; // Пропускаем, если слот в партии пуст
            ...
            {
                memberUIComponent.Setup(memberStats); // PartyMemberUI сам подпишется на все нужные события CharacterStats
            ...
            else
            {
                ...
                Destroy(slotInstance); // Удаляем некорректно созданный слот
            }
        }
    }
